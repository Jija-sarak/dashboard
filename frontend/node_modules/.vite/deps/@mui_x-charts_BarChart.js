...
import "./chunk-GILWY5VU.js";
import {
  useThemeProps
} from "./chunk-Q7M3XG4H.js";
import {
  clsx_default,
  composeClasses,
...
function HighlightedProvider({
  ...
  const series = useSeries();
  const seriesById = React24.useMemo(() => {
  ...
  const providerValue = React24.useMemo(() => {
    return {
      isInitialized: true,
      data: {
        highlightScope,
...
function useHighlighted() {
  const {
    isInitialized,
    data
  } = React25.useContext(HighlightedContext);
  if (!isInitialized) {
...
var AnimationContext = React36.createContext({
  isInitialized: false,
  data: {
...
function AnimationProvider(props) {
  ...
  const [skipAnimation2, setSkipAnimation] = React37.useState(isAnimationDisabledEnvironment || void 0);
  useIsomorphicLayoutEffect(() => {
    var _a;
...
function useSkipAnimation(skipAnimation2) {
  const {
    isInitialized,
    data
  } = React38.useContext(AnimationContext);
  if (!isInitialized) {
...
var useAxisEvents = (disableAxisListener) => {
  ...
  const mousePosition = React40.useRef({
    isInChart: false,
    x: -1,
    y: -1
  });
...
var ChartsSurface = React41.forwardRef(function ChartsSurface2(inProps, ref) {
  const props = useThemeProps({
    props: inProps,
    name: "MuiChartsSurface"
  });
...
function ChartsPiecewiseGradient(props) {
  ...
  return (0, import_jsx_runtime16.jsx)("linearGradient", {
    ...
    children: colorMap.thresholds.map((threshold2, index2) => {
      ...
      return (0, import_jsx_runtime16.jsxs)(React42.Fragment, {
...
function useChartGradient() {
  const {
    chartId
  } = React44.useContext(DrawingContext);
  return React44.useCallback((axisId, direction) => `${chartId}-gradient-${direction}-${axisId}`, [chartId]);
}
...
var useDefaultizeAxis = (inXAxis, inYAxis, dataset) => {
  const xAxis = React45.useMemo(() => defaultizeAxis(inXAxis, dataset, "x"), [inXAxis, dataset]);
  const yAxis = React45.useMemo(() => defaultizeAxis(inYAxis, dataset, "y"), [inYAxis, dataset]);
  return [xAxis, yAxis];
};
...
var ResponsiveChartContainer = React49.forwardRef(function ResponsiveChartContainer2(props, ref) {
  const {
    hasIntrinsicSize,
    chartContainerProps,
    resizableChartContainerProps
